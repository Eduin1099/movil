...
const DepartamentoDetailScreen = ({ route, navigation }) => { 
  ...
  // Función para cargar los datos del departamento y trabajadores
  const loadData = async () => { 
    try { 
      const deptoData = await getDepartamento(departamentoId); 
      setDepartamento(deptoData); 
       
      // Cargar trabajadores asociados al departamento
  ...
  // Ejecutar cuando el componente se monte o el id del departamento cambie
  useEffect(() => { 
    loadData(); 
  }, [departamentoId]); 
  ...
  if (loading) { 
    return <Loading />; // Mostrar componente de carga si los datos están siendo recuperados
  } 
